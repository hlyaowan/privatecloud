...
public class PtcAlipayServiceImpl implements PtcAlipayService {
	...
	public Wrapper pay(String orderNo, LoginAuthDto loginAuthDto) {
		...
		OrderDto order = omcOrderService.queryOrderDtoByUserIdAndOrderNo(
				userId, orderNo);
		if (order == null) {
		...
		List<OmcOrderDetail> orderItemList = omcOrderDetailService
				.getListByOrderNoUserId(orderNo, userId);
		for (OmcOrderDetail orderItem : orderItemList) {
			GoodsDetail goods = GoodsDetail.newInstance(
					orderItem.getProductId().toString(),
					orderItem.getProductName(),
					BigDecimalUtil
							.mul(orderItem.getCurrentUnitPrice().doubleValue(),
		...
		case SUCCESS:
			...
			// 细节细节细节
			String qrPath = String.format(qrCodePcPath + "/qr-%s.png",
			...
			optUploadFileReqDto
					.setUploadFileByteInfoReqDto(optUploadFileByteInfoReqDto);
			OptUploadFileRespDto optUploadFileRespDto = null;
			try {
				optUploadFileRespDto = opcOssService
	...
	public Wrapper aliPayCallback(Map<String, String> params) {
		...
		if (PtcApiConstant.AlipayCallback.TRADE_STATUS_TRADE_SUCCESS
				.equals(tradeStatus)) {
